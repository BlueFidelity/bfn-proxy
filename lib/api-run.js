...
function SIGKILL(sig_type,sig_term) {
	console.error('Received kill signal (' + sig_type + ')...');
}

process.on('SIGTERM',function(){SIGKILL('SIGTERM',true);});
//process.on('SIGINT',function(){SIGKILL('SIGINT',true);});
process.on('SIGBREAK',function(){SIGKILL('SIGBREAK',true);});
process.on('SIGHUP',function(){SIGKILL('SIGHUP',true);});
process.on('SIGPIPE',function(){SIGKILL('SIGPIPE',false);});
process.on('SIGUSR1',function(){SIGKILL('SIGUSR1',false);});
...
//	if (!id) return resEndJSON(res, 400, 'Bad Request');
	...
	if (!callback_url) {
		...
		}, Math.min(Number(opts.wait) || 15000, 30000));
	}
	if (opts.exec) {
