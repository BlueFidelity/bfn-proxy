...
module.exports = function (opts) {
  ...
  var omitHeaders = ['request', 'host', 'connection', 'forwarded', 'x-appengine-user-ip', 'x-forwarded-for', 'x-forwarded-proto', 'x-forwarded-port', 'x-request-start', 'x-request-id']
  ...
  var pickByHeaders = typeof opts['pickByHeaders'] === 'undefined' ? function (k) {
    return !/^(?:cdn-loop|early-data|ipcountry|traceparent|function-execution-id|fastly-.*|cf-.*|x-(?:country|real|cloud|appengine|forwarded|firebase).*)$/.test(k)
  } : opts['pickByHeaders']
  return function (request, response) {
    ...
    if (pickByHeaders) headers = pickBy(headers, pickByHeaders)
        ...
        if ((res.statusCode >= 200 && res.statusCode <= 299) || res.statusCode === 304) {
          ...
          if (~[301, 302, 303, 307, 308].indexOf(res.statusCode) && res.headers.location) {
            if (request.headers.host) {
              if (res.headers.location.indexOf('/') === 0) res.headers.location = (request.headers['x-forwarded-proto'] || 'http') + '://' + request.headers.host + '/' + (opts.baseUrl || '') + '/http' + (scheme === 'http' ? '' : 's') + '://' + options.host + res.headers.location
              else if (/^https?:\/\//i.test(res.headers.location)) res.headers.location = (request.headers['x-forwarded-proto'] || 'http') + '://' + request.headers.host + '/' + (opts.baseUrl || '') + '/' + res.headers.location.replace(/^\w+:\/\/[^/]+/, function (match) { return match.replace(/:\d+$/, '').toLowerCase() })
